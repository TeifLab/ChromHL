...
try
    ...
    for kk=((job_id-1)*100):((job_id*100)-1)
        identifier=pad(num2str(kk),4,'left','0');
        Seq_DNA=Read_FASTA_all(['/home/gthorn/teif/graeme/Intersect_Jenuwein_MCORE_peaks/WT_not_DKO/WT_not_DKO_peaks/FASTA/region_WT_not_DKO_' identifier '*.fa']);
        if ~isempty(Seq_DNA)
            Driver;
